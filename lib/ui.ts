...
export function printHelp() {
  ...
  console.log(`  ${c.prompt('/review')}  ${c.dim('Run adversarial code review on active diff')}`);
  console.log(`  ${c.prompt('/review-diff')} ${c.dim('Review the full diff and accept or discard it')}`);
  console.log(`  ${c.prompt('/explain')} ${c.dim('Explain a file or code section')}`);
  console.log(`  ${c.prompt('/fix')}     ${c.dim('Fix a bug or error in the codebase')}`);
  console.log(`  ${c.prompt('/tests')}   ${c.dim('Write or run tests for a target')}`);
  console.log(`  ${c.prompt('/index')}   ${c.dim('Build the semantic search index')}`);
  console.log(`  ${c.prompt('/gh')}      ${c.dim('Run a GitHub CLI command (pr/issue/comment)')}`);
  console.log(`  ${c.prompt('/permissions')} ${c.dim('Reload and show permission rules')}`);
  console.log(`  ${c.prompt('/agents')}   ${c.dim('List available sub-agents')}`);
  console.log(`  ${c.prompt('/mcp')}     ${c.dim('List configured MCP servers')}`);
